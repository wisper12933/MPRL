"""RolloutEngine implementation using a local HuggingFace model (TRL backend)."""
...
class TrlEngine(RolloutEngine):
    """
    Local HF model rollout for agent training without Ray.

    Generation runs synchronously on GPU inside a thread pool so the async
    agent execution engine can stay non-blocking.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.device = device or (model.device if hasattr(model, "device") else "cuda")
