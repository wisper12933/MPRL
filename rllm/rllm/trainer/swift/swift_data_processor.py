"""Data processing for Swift-based agent RL training."""
...
class SwiftSampleBuilder:
    @staticmethod
    def _is_prefix(seq1: list[int], seq2: list[int]) -> bool:
        return len(seq1) <= len(seq2) and seq2[: len(seq1)] == seq1

    @staticmethod
    def build_from_trajectory(trajectory: Trajectory, advantage: float) -> list[SwiftTrainingSample]:
        ...
        class SequenceAccumulator:
            ...
            def to_sample(self) -> SwiftTrainingSample:
        ...
        datums: list[SwiftTrainingSample] = []
        accumulator = SequenceAccumulator()

        for step in trajectory.steps:
            if accumulator.is_empty():
                accumulator.add_step(step, advantage, is_extension=False)
            elif SwiftSampleBuilder._is_prefix(accumulator.full_sequence, step.prompt_ids):
...
def process_episodes(
    episodes: list,
    advantage_computer: SwiftAdvantageComputer,
    trajectory_filter: SwiftTrajectoryFilter,
    algorithm_config,
) -> tuple[list[SwiftTrainingSample], dict]:
    ...
    samples: list[SwiftTrainingSample] = []
    ...
    for group in groups:
        ...
        for traj, advantage in zip(group.trajectories, advantages, strict=False):
            samples.extend(SwiftSampleBuilder.build_from_trajectory(traj, advantage))
