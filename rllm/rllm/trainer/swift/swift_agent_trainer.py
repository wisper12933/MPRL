"""Swift-based (ms-swift style) trainer for rLLM agents (no Ray).
...
class SwiftAgentTrainer:
    """Agent RL trainer with ms-swift-style vLLM/batched rollout + Accelerate training."""
    ...
    def __init__(
        ...
        self.trainer = SwiftPolicyTrainer(config=config)
        ...
        mode = rollout_cfg.get("mode", "server")
        # server mode does not need the train-model on GPU for generate
        model_for_rollout = None if mode == "server" else self.trainer.get_model()
        ...
        self.agent_execution_engine = AsyncAgentExecutionEngine(
            ...
            rollout_engine_args={
                "model": model_for_rollout,
                "tokenizer": self.tokenizer,
                "mode": mode,
                "model_name": self.config.model.name,
                "base_url": rollout_cfg.get("base_url", "http://127.0.0.1:8000/v1"),
                "api_key": rollout_cfg.get("api_key", "EMPTY"),
                "max_prompt_length": self.config.data.max_prompt_length,
                "max_response_length": self.config.data.max_response_length,
                "sampling_params": sampling_params,
                "disable_thinking": self.config.get("disable_thinking", False),
                "device": self.trainer.device,
                "batch_size": rollout_cfg.get("batch_size", 8),
    ...
    def convert_to_episode(self, group: list) -> Episode:
        ...
        for traj in group:
            ...
            trajectories.append(Trajectory(steps=steps, reward=traj["trajectory_reward"]))
